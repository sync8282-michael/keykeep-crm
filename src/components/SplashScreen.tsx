...
export function SplashScreen({ onComplete }: SplashScreenProps) {
  const [phase, setPhase] = useState<"shield" | "drawing" | "filling" | "zooming" | "done">("shield");

  useEffect(() => {
    // Phase 0: Shield emerges (0.8s)
    const drawTimer = setTimeout(() => setPhase("drawing"), 800);
    // Phase 1: Drawing animation (1.5s)
    const fillTimer = setTimeout(() => setPhase("filling"), 2300);
    // Phase 2: Fill in colors (0.6s)
    const zoomTimer = setTimeout(() => setPhase("zooming"), 2900);
  ...
  const showDrawing = phase === "drawing" || phase === "filling" || phase === "zooming";
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col items-center gap-8 transition-transform duration-700 ease-out ${
          phase === "zooming" ? "scale-[2.5]" : "scale-100"
        }`}
      >
        {/* Shield Logo with emerge animation */}
        <div
          className={`shield-container transition-all duration-700 ease-out ${
            phase === "shield" ? "shield-emerge" : "shield-settled"
          ...
          >
            <g clipPath="url(#clip0_1_2)">
              ...
              <path
                ...
                className={`splash-path ${phase === "filling" || phase === "zooming" ? "filled" : ""}`}
                style={{ strokeDasharray: 400, strokeDashoffset: 400, animationDelay: "0.1s" }}
              />
              
              {/* e (first) */}
              <path
                d="M86.9023 18.885C108.836 16.9102 117.175 31.0149 116.561 50.8098C103.324 50.7298 90.0865 50.7402 76.8496 50.8352C77.4041 52.3051 78.1447 54.2603 78.9932 55.5402C81.2117 58.9 84.6859 61.2252 88.6357 62.0002C95.436 63.355 101.24 61.1294 106.809 57.5295C109.169 60.7495 110.747 62.4797 113.506 65.3996C88.7593 87.9096 49.657 61.3194 68.3525 30.1994C72.4841 23.3197 79.486 20.4449 86.9023 18.885ZM104.024 41.9699C101.924 31.9101 96.5285 29.4602 86.7978 30.3801C80.9328 32.9651 79.3924 34.4501 76.9394 40.7551C76.797 41.1199 76.6574 41.4897 76.5215 41.8645L95.2783 41.965L104.024 41.9699Z"
                className={`splash-path ${phase === "filling" || phase === "zooming" ? "filled" : ""}`}
                style={{ strokeDasharray: 350, strokeDashoffset: 350, animationDelay: "0.15s" }}
              />
              
              {/* e (second) */}
              <path
                d="M340.461 18.8796C362.686 17.0549 371.261 30.69 370.656 50.9197C357.486 51.0997 343.591 50.3552 330.516 51.0652C336.466 64.78 348.851 64.925 360.461 57.655C362.706 60.2949 364.931 62.9547 367.131 65.6296C356.916 74.1646 342.066 76.4598 330.356 69.5798C324.311 66.0499 319.996 60.1751 318.446 53.3552C314.511 36.7802 323.541 22.3796 340.461 18.8796ZM342.536 29.8953C335.081 31.8653 332.741 34.3504 330.501 41.8552L346.951 41.9246L358.29 41.9197C356.22 32.99 351.825 29.5703 342.536 29.8953Z"
                className={`splash-path ${phase === "filling" || phase === "zooming" ? "filled" : ""}`}
              ...
              {/* e (third) */}
              <path
                d="M277.771 18.8905C279.806 18.6705 281.851 18.5902 283.891 18.6502C302.751 19.2002 309.336 34.3854 308.431 50.8954C295.251 51.1454 281.546 50.5045 268.521 51.0545C268.961 52.0745 269.601 53.6048 270.146 54.5848C272.236 58.2548 275.761 60.8854 279.876 61.8504C286.326 63.4103 292.461 61.4747 297.956 58.1698L298.266 58.0653C299.586 58.4804 304.036 64.0401 305.331 65.5301C289.551 79.0001 262.461 75.3501 256.701 54.0702C252.176 37.3603 261.141 23.0906 277.771 18.8905ZM281.076 29.9452C273.526 31.3402 270.711 34.5004 268.516 41.8104L283.931 41.8446H296.211C293.541 33.7049 290.581 29.6353 281.076 29.9452Z"
                className={`splash-path ${phase === "filling" || phase === "zooming" ? "filled" : ""}`}
                style={{ strokeDasharray: 350, strokeDashoffset: 350, animationDelay: "0.25s" }}
              />
              ...
              <path
                ...
                className={`splash-path ${phase === "filling" || phase === "zooming" ? "filled" : ""}`}
                style={{ strokeDasharray: 600, strokeDashoffset: 600, animationDelay: "0.35s" }}
              />
            </g>
            <defs>
              <clipPath id="clip0_1_2">
                <rect width="438" height="111" fill="white"/>
              </clipPath>
            </defs>
          </svg>
        </div>
      ...
      <style>{`
        ...
        .shield-emerge {
          ...
          animation: shieldReveal 0.8s cubic-bezier(0.34, 1.56, 0.64, 1) forwards;
        }
